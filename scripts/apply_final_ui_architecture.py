...
# Remove legacy interaction helpers. There must be one canonical controller.
...
# Repair the canonical support filter logic inside the main app IIFE.
...
filter_fn="""function setSupportFilter(key,checked){const root=$('supportFilters'),all=root&&root.querySelector('input[data-filter=\\\"all\\\"]'),others=root?Array.from(root.querySelectorAll('input:not([data-filter=\\\"all\\\"])')):[];const keys=others.map(i=>i.dataset.filter).filter(Boolean);if(key==='all'){if(checked){supportFilters=new Set(['all'])}else{supportFilters=new Set()} }else{if(supportFilters.has('all'))supportFilters=new Set(keys);checked?supportFilters.add(key):supportFilters.delete(key)}const everySelected=keys.length>0&&keys.every(k=>supportFilters.has(k));if(!supportFilters.size||everySelected)supportFilters=new Set(['all']);root?.querySelectorAll('input').forEach(i=>{const isAll=i.dataset.filter==='all';i.checked=isAll?supportFilters.has('all'):supportFilters.has('all')||supportFilters.has(i.dataset.filter);i.parentElement?.classList.toggle('selected',i.checked)});renderSupports()}\n"""
s=s[:start_filter]+filter_fn+s[end_filter:]

# Replace the previous final UI runtime with one that uses the canonical controls.
...
(function(){
  ...
  function boot(){
    ...
    // Move canonical preparation cards into one detail surface; do not clone them.
    const cards={};
    ...
    // Use only the canonical handler for support filters. The runtime normalizes UI
    // state at startup and leaves behavior to setSupportFilter/bindControls.
    const sf=byId('supportFilters');
    if(sf){
      const all=sf.querySelector('input[data-filter="all"]'),others=Array.from(sf.querySelectorAll('input:not([data-filter="all"])'));
      others.forEach(function(i){i.addEventListener('change',function(){
        if(others.every(function(x){return x.checked}))all.checked=true;
        else all.checked=false;
        if(typeof setSupportFilter==='function')setSupportFilter(i.dataset.filter,i.checked);
      })});
      all.addEventListener('change',function(){if(typeof setSupportFilter==='function')setSupportFilter('all',all.checked)});
      if(typeof setSupportFilter==='function')setSupportFilter('all',true);
    }

    // One global menu, using the same preparation controls and no duplicate grid.
    ...
    if(top&&!byId('cpMenuBtn')){
      ...
      panel.addEventListener('click',function(e){const m=e.target.dataset.menu;if(!m)return;panel.classList.remove('open');if(m==='routes')openConfig('route');if(m==='support')openConfig('support');if(m==='walk')byId('cpStart').click();if(m==='diary'){const n=byId('addNoteBtn');if(n)n.click()}if(m==='settings'){if(cards.notifications)cards.notifications.style.display='block';openConfig('notifications')}if(m==='help')showInfo('Ajuda','Consulte a preparação, o GPS e os apoios antes de iniciar.');if(m==='contact')showInfo('Contacto','Use o formulário de contacto da aplicação.');if(m==='about')showInfo('Sobre','Caminhos do Peregrino — aplicação de apoio à caminhada.')});
    }
...
print('Canonical preparation architecture applied')