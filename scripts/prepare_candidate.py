...
def patch_index() -> None:
    ...
    if old_route_store not in html: raise RuntimeError('Route store signature not found')
    ...
    if old_select not in html: raise RuntimeError('selectRoute signature not found')
    ...
    if 'function makeTestSupports(' not in html:
        ...
        if marker not in html: raise RuntimeError('renderSupports marker not found')
...
for name,url in route_sources.items():
 dst=ROUTES/name; download(url,dst); validate_route(dst); print(f'{name}: {len(dst.read_bytes())} bytes, sha256={hashlib.sha256(dst.read_bytes()).hexdigest()}')
