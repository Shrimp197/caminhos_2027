...
internal fun PreparationExperienceV3(route: Route, routeOptions: List<AndroidRouteOption>, selectedRouteId: String, onSelectRoute: (String) -> Unit, onConfirm: (Double, Double, WalkingPreparationConfig) -> Unit, onBack: () -> Unit) {
    ...
    when (screen) {
        ...
        else -> PreparationHomeV3(route, config, notes.size, onRoutes = { screen = "routes" }, onRange = { screen = "range" }, onBreaks = { screen = "breaks" }, onAudio = { screen = "audio" }, onSupports = { screen = "supports" }, onNotes = { screen = "notes" }, onConfirm = { if (startKm >= 0.0 && destinationKm <= route.totalDistanceKm && startKm < destinationKm) onConfirm(startKm, destinationKm, config.copy(notes = notes.toList())) }, onBack = onBack)
    }
}
...
private fun PreparationHomeV3(route: Route, config: WalkingPreparationConfig, notesCount: Int, onRoutes: () -> Unit, onRange: () -> Unit, onBreaks: () -> Unit, onAudio: () -> Unit, onSupports: () -> Unit, onNotes: () -> Unit, onConfirm: () -> Unit, onBack: () -> Unit) {
    Column(Modifier.fillMaxSize().background(PSurface).verticalScroll(rememberScrollState()).padding(horizontal = 16.dp, vertical = 10.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) { Tile(Icons.Filled.LocationOn, "Início e fim", Modifier.weight(1f), onRange); Tile(Icons.Filled.Headphones, "Áudio", Modifier.weight(1f), onAudio); Tile(Icons.Filled.Map, "Orientação / Pausas", Modifier.weight(1f), onBreaks) }
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) { Tile(Icons.Filled.Place, "Apoios", Modifier.weight(1f), onSupports); Tile(Icons.Filled.Notes, "Notas", Modifier.weight(1f), onNotes); Spacer(Modifier.weight(1f).height(126.dp)) }
...
@Composable private fun SupportsSub(onBack: () -> Unit) { PrepScaffold("Apoios", "Consulte os tipos de apoio disponíveis ao longo do percurso.", onBack) { listOf("Água", "Alimentação", "WC", "Alojamento").forEach { label -> Card(Modifier.fillMaxWidth(), RoundedCornerShape(14.dp), border = BorderStroke(1.dp, PBorder), colors = CardDefaults.cardColors(containerColor = Color.White)) { Row(Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) { Icon(Icons.Filled.Place, null, tint = PBlue); Text(label, Modifier.padding(start = 12.dp), color = PBlue, fontWeight = FontWeight.Bold) } } }; Text("A localização detalhada dos apoios é apresentada durante a caminhada.", color = PMuted) } }
