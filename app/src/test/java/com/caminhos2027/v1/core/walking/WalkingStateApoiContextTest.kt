...
class WalkingStateApoiContextTest {
    ...
    @Test fun nextApoiIsCalculatedByRouteDistance() {
        val walk = Walk("w", "sr-route", 0.0, 2.0, 0.0, null, null, null, WalkStatus.ACTIVE, listOf("stage-1", "stage-2"))
        val apoi = Apoi(
            id = "water-1",
            name = "Água SR",
            description = null,
        ...
        val state = WalkingStateBuilder.build(
            route, walk, GpsState.ON_ROUTE,
            RoutePosition("sr-route", 0.8, 2.0, "stage-1", PositionConfidence.HIGH),
            listOf(apoi)
        )
        assertNotNull(state.nextApoi)
        assertEquals("water-1", state.nextApoi!!.id)
        assertEquals(0.4, state.nextApoiDistanceKm!!, 0.001)
    }
}
